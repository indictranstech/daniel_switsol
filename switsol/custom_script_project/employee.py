...
def get_approver_role(user_id,user_type):
	...
	if user_type == "Approver" and user_type not in [role[0] for role in roles]:
		...
		if user_type not in [role[0] for role in roles]:
			return "User <b><a href="+href+" target='blank'>{0}</a></b> has not {1} Role<br>Please Add <b>{1}</b> In User Roles".format(user_id,user_type) 	
		elif user_type in [role[0] for role in roles] and "Leave Approver" not in [role[0] for role in roles]:	
			return "User <b><a href="+href+" target='blank'>{0}</a></b> has not {1} Role<br>Please Add <b>{1}</b> and <b>Leave Approver</b> In User Roles\
			<br>If you want add this user as Executor".format(user_id,user_type)
