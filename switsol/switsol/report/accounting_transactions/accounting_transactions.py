...
def get_columns(filters):
	columns = [
		...
		_("Debit") + ":Float:100", _("Credit") + ":Float:100",_("Voucher") + "::110"
	]
...
def get_gl_entries(filters):
	...
	gl_entries = frappe.db.sql("""
		select
			...
			sum(debit) as debit, sum(credit) as credit,voucher,
...
def get_result_as_list(data, filters):
	result = []
	for d in data:
		row = [d.get("posting_date"), d.get("account"), d.get("debit"), d.get("credit"),d.get('voucher')]
