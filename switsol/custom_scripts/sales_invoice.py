...
def payment_reminder(customer_name,args,flag,reminder_count,si_name):
	...
	if flag == 'Reminder':
		...
		letter_name = make_new_letter(si_doc,reminder_count,data)
...
def make_new_letter(si_doc,reminder_count,data):
	...
	letter.status = "Sent"
	letter.customer_address = si_doc.customer_address
	...
	letter.contact_greeting = frappe.db.get_value("Customer",si_doc.customer,"greeting")
	letter.letter_text = data.get('predefined_text_container')
	letter.body_text = data.get('predefined_text')
